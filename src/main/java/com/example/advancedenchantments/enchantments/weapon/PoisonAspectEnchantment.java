...
public class PoisonAspectEnchantment extends BaseEnchantment {
	public static final PoisonAspectEnchantment INSTANCE = new PoisonAspectEnchantment();
	
	private PoisonAspectEnchantment() {
	...
	public void onTargetDamaged(LivingEntity user, LivingEntity target, int level) {
		if (!target.getWorld().isClient && target.isAlive()) {
			target.addStatusEffect(new StatusEffectInstance(StatusEffects.POISON, 100 * level, level - 1));
		}
	}
}
